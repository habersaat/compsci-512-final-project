...
from NetworkServer import NetworkServer as Server
...
def handle_candidate(server):
    ...
    if time.time() >= server._state._timeoutTime:
        ...
        print(f"Server {server._name} timed out as a candidate")
        ...
        while True:
            ...
            with self.lock:
                if leader_id is not None and leader_id != self.last_known_leader:
                    print(f"IMPORTANT: New leader elected: {leader_id}. Last known leader: {self.last_known_leader}")
